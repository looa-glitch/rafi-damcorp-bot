...
class helper {
    ...
    static async logInfo(str, ...any){
        ...
        console.log('\n[INFO]', this.timenow().toString(), "-", `app=${pjson.name}[${pjson.version}]`, "-", `session=${reqId}`, "-", str, ...any)
    }

    static async logRequestIn(req){
        let reqId = session.reqId || "[no session]"
        console.log('\n[INFO]', this.timenow().toString(), "-", `app=${pjson.name}[${pjson.version}]`, "-", `session=${reqId}`, "-", `Incoming request to ${req.path} [${req.method}] with headers={ ${this.jsonstr(req.headers)} }, params={ ${this.jsonstr(req.params)} }, query={ ${this.jsonstr(req.query)} }, body={ ${this.jsonstr(req.body)} }`)
    }
